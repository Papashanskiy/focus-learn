...
- Calibration baseline уже умеет выбирать до 5 accepted questions по разным competencies и запускать из Today empty-state mixed practice session, которая идет по выбранному service-level плану; baseline/mock/Today practice selection теперь отдает приоритет accepted canonical `must-know` questions после weak-repeat/answer-count signals, чтобы first exposure шел через curated pack без потери weak topic repetition. Outcome сохраняется с типом `calibration_baseline`, чтобы readiness/trend отличал первичную оценку от обычной practice. Repeat baseline через 7 дней показывается как Today/readiness action и после завершения сохраняет сравнение readiness delta с предыдущей baseline. Mock senior interview запускается без ручного выбора topic из Today/readiness, смешивает coding/theory/system design/debugging и показывает section progress в practice review flow.
...
- [x] TUI UX inventory: documented current first-screen panes/actions, command groups and the minimal-vs-advanced split in `TUI_UX_INVENTORY.md`.
- [x] TUI main menu foundation: start screen now has a selectable mode menu for Today, Practice, Learn, Mock Interview, System Design, Readiness and Settings with keyboard/mouse routing.
- [x] TUI minimal start shell: default minimal start screen now hides the generic right/debug pane and stops advertising `/content`, `/materials` and `/questions-review` in first-screen hints while keeping Today primary action and manual topic selection.
- [x] 0B docs sync: README/CLAUDE/DEVELOPMENT_LOG/ROADMAP now document automatic content retry/backoff, empty legacy CLI session outcome and schema-version guard policy.
- [x] Content generation automatic retry: transient worker failures now requeue with retry/backoff until `max_attempts`; final/non-retryable failures remain `failed`, and CLI/TUI worker output reports scheduled retries explicitly.
- [x] CLI empty session status: legacy CLI `session` now marks `/quit` before the first saved answer as `abandoned`, keeping it out of completed stats/readiness counters.
- [x] Migration schema-version guard: `CURRENT_SCHEMA_VERSION` is aligned with the latest explicit migration step and covered by a regression test against future drift.
- [x] Source curation docs sync: README/roadmap/log now describe source refresh/candidates, automated curation policy, audit/undo surfaces, canonical coverage and canonical priority in practice selection.
- [x] Questions-review happy path copy: CLI/TUI `questions-review` now frames automated curation as the happy path and accept/archive as manual audit overrides.
- [x] Questions-review audit context: CLI/TUI `questions-review` now shows latest auto-curation rationale, source evidence and safe undo hint for pending questions with saved audit rows.
- [x] TUI questions-review observability: `/questions-review` now shows deterministic quality flags plus source URL/retrieved/category/frequency metadata for pending generated questions without changing accept/archive behavior.
- [x] Questions review CLI observability: `questions-review` now shows deterministic quality flags plus source URL/retrieved/category/frequency metadata for pending generated questions without changing accept/archive behavior.
- [x] Curriculum fallback: `fallback_questions()` now returns concrete canonical-style incident/design scenarios instead of generic topic templates, with specificity regression coverage.
- [x] Content quality gate: background and curriculum-generated questions with known generic prompt wording are archived at creation time while normal generated questions still enter pending review.
...
- [x] Canonical metadata tags: `seed_defaults()` now links accepted `canonical-2026` questions to existing tag storage for must-know, high-frequency and top-level interview type metadata without adding schema.
- [x] Practice selection canonical priority: baseline, mock interview and Today practice ordering now prefer accepted canonical must-know questions after weak-repeat/answer-count signals, preserving weak topic repetition.
...
- [x] Canonical metadata: хранить для seeded questions frequency/type/competency metadata через существующие tags/competencies; добавлять новую схему только если tags не покрывают `must_know`, `coding`, `api`, `db`, `system_design`.
- [x] Practice selection: при first-run baseline, Today drill и mock interview отдавать приоритет accepted canonical must-know questions перед generated accepted questions, сохраняя повторение weak topics.
- [x] Content quality gate: отклонять или отправлять на review generated prompts с generic wording вроде "ключевой production-риск", "backend-flow" или "какие tradeoffs" без конкретного scenario, constraints и ожидаемых mechanisms.
- [x] Curriculum fallback: заменить generic `fallback_questions()` templates на конкретные canonical-style fallback questions и добавить regression-тесты на специфичность fallback prompts.
- [x] Curation observability UX: переделать CLI/TUI `questions-review` в audit/undo surface, где видны quality flags, source, frequency metadata и LLM rationale, но ручной approve не является обязательным happy path.
  - [x] CLI questions-review observability: показывать pending questions с deterministic quality flags, source URL/retrieved metadata, category hints и frequency hint без изменения accept/archive behavior.
  - [x] TUI questions-review observability: показывать те же quality/source/frequency metadata в `/questions-review` без изменения accept/archive behavior.
  - [x] Questions-review audit context: добавить latest curator rationale/source evidence и безопасную undo-подсказку для вопросов, где есть сохраненный auto-curation audit.
  - [x] Questions-review happy path copy: переименовать/переформулировать surface так, чтобы automated curation был основным flow, а ручной accept/archive выглядел как audit action.
- [x] Docs: после source refresh, auto-curation, canonical pack и quality gate обновить `README.md`, `DEVELOPMENT_LOG.md` и `ROADMAP.md`: описать refresh/audit flow, источники покрытия, automated approval policy и приоритет canonical questions.

### 0B. Review follow-up: reliability and migration hygiene

- [x] Migrations: поднять `CURRENT_SCHEMA_VERSION` до последней migration step и добавить regression-тест, который падает при рассинхронизации schema version и migrations.
- [x] Content generation retry: привести retry/backoff к одному контракту - automatic requeue transient failures until `max_attempts` или explicit manual-only retry без неиспользуемого backoff state; предпочтительно automatic retry для TUI worker.
- [x] CLI session status: завершать пустой legacy CLI `session` через `abandon_if_empty=True` и добавить regression-тест, что `/quit` без ответов не влияет на stats/readiness как completed session.
- [x] Docs: если меняются retry semantics, CLI session outcome или schema-version policy, обновить `README.md`, `DEVELOPMENT_LOG.md` и этот roadmap в той же итерации.

### 0C. Low-friction TUI: minimal mode and game-like menu

- [x] TUI UX inventory: перечислить текущие first-screen actions, panes, slash commands and secondary surfaces; отметить, какие элементы должны уйти из default minimal flow в advanced/debug.
- [x] Main menu foundation: добавить game-like стартовое menu с выбором режима `Today`, `Practice`, `Learn`, `Mock Interview`, `System Design`, `Readiness`, `Settings` через arrows/Enter и мышь.
...
- Вопросная база теперь имеет first-run `canonical-2026` pack на 40 accepted must-know questions; generated accepted questions все еще могут быть слишком общими, поэтому quality gate, canonical priority, curation observability и docs sync закрыты в `0A`, а дальнейшая надежность очереди идет в `0B`.
...
- Generated questions/materials/scenarios уже сохраняются; generated questions проходят консервативный same-topic duplicate check и quality gate по generic wording. Source-backed candidates могут идти через automated curation и audit/undo, а background LLM generated questions все еще остаются pending-review audit exceptions до отдельной auto-curation интеграции.
...
- Старый CLI `session` не спрашивает self-score и не участвует в новом rubric workflow; TUI остается основным интерфейсом practice. Пустые legacy CLI sessions уже завершаются как `abandoned` и не попадают в completed stats.
