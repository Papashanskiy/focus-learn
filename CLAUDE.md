...
TUI должен адаптироваться к режиму работы. Default flow теперь идет через minimal mode: стартовый экран показывает Today action и mode menu, practice/learning/system design держат текущую работу в центральной области, а служебные очереди, audits, raw history и queue controls остаются в Advanced/power-user surfaces.
...
- Default TUI visual mode — minimal: start screen показывает Today recommendation, один primary action, compact readiness/progress context и mode menu; practice, learning и system design скрывают постоянные side panels и держат next action/context в центральной области.
